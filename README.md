...
- 33일차(JAVA) - 둘만의 암호 - 복잡한 문제였다.... 테스트 코드에서 실패한 경우가 2번 생겼는데 첫 번째는 z에서 a로 돌아가는 경우에도 skip에 있는 문자열이 나오는 경우를 고려하지 않아서 였다. 2번째로는 z를 넘어선 값인 경우에는 -z를 해주는 것으로 해결을 봤는데 그 후에도 여전히 z값을 넘는 경우가 생길 수도 있다는 것이였다. 따라서 if문을 하나 더 넣어주자 풀렸다...
